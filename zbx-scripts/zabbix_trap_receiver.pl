#!/usr/bin/perl
#
# Zabbix Copyright (C) 2001-2013 Zabbix SIA
#
...
#########################################
# This is an embedded perl SNMP trapper receiver designed for sending data to the server. The receiver will pass the received SNMP traps to Zabbix server or proxy running on the same 
# machine. Please configure the server/proxy accordingly.
#
...
#       Temporary file used for passing data to the server (or proxy). Must be the same as in the server (or proxy) configuration file.
#
# Mandatory: yes Default:
$SNMPTrapperFile = '/tmp/zabbix_traps.tmp';
### Option: DateTimeFormat
#       The date time format in strftime() format. Please make sure to have a corresponding log time format for the SNMP trap items.
#
# Mandatory: yes Default:
...
###################################
use Fcntl qw(O_WRONLY O_APPEND O_CREAT); use POSIX qw(strftime); sub zabbix_receiver {
        my (%pdu_info) = %{$_[0]};
        my (@varbinds) = @{$_[1]};
        ...
        # print trap header
        #       timestamp must be placed at the beggining of the first line (can be omitted) the first line must include the header "ZBXTRAP [IP/DNS address] "
        #              * IP/DNS address is the used to find the corresponding SNMP trap items * this header will be cut during processing (will not appear in the item value)
        printf OUTPUT_FILE "%s ZBXTRAP %s\n", strftime($DateTimeFormat, localtime), $hostname;
        #printf OUTPUT_FILE "%s %s - ZBXTRAP %s\n", strftime($DateTimeFormat, localtime), "$hostname", $hostname;
        # print the PDU info
